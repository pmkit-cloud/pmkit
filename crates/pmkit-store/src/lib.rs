...
pub use source::{
    ...
    FixtureCanonicalLogSource, ProviderIdentity, QuorumVerifiedFinalizedLogBatch, RawLogIdentity,
    RawRpcLog, agree_on_finalized_heads, agree_on_finalized_log_batches,
    verify_block_header_linkage,
};
...
pub trait TapeStore: Send + Sync {
    ...
    /// Lists durable intents accepted by the venue for one owner scope.
    async fn read_accepted_intents(
        ...
        Err(StoreError::Storage {
            message: "accepted-intent recovery is not configured".into(),
        })
    }
